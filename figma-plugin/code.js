 ...
 */
async function createDesignNodes(designData) {
    const createdNodes = [];
...
async function createNode(nodeData) {
    ...
    if (nodeData.fills && "fills" in node) {
        const fills = nodeData.fills.map((fill) => {
    ...
    if (nodeData.strokes && "strokes" in node) {
        const strokes = nodeData.strokes.map((stroke) => {
    ...
    if (nodeData.effects && "effects" in node) {
        const effects = nodeData.effects.map((effect) => {
            var _a;
            if (effect.type === "DROP_SHADOW" || effect.type === "INNER_SHADOW") {
                return {
                    type: effect.type,
                    color: effect.color
                        ? { r: effect.color.r, g: effect.color.g, b: effect.color.b, a: (_a = effect.color.a) !== null && _a !== void 0 ? _a : 1 }
                        : { r: 0, g: 0, b: 0, a: 0.25 },
    ...
    if (node.type === "FRAME" && nodeData.autoLayout) {
        ...
        if (al.primaryAxisSizingMode)
            node.primaryAxisSizingMode = al.primaryAxisSizingMode;
        if (al.counterAxisSizingMode)
            node.counterAxisSizingMode = al.counterAxisSizingMode;
        if (al.primaryAxisAlignItems)
            node.primaryAxisAlignItems = al.primaryAxisAlignItems;
        if (al.counterAxisAlignItems)
            node.counterAxisAlignItems = al.counterAxisAlignItems;
        if (al.paddingLeft)
        ...
        if (al.itemSpacing)
            node.itemSpacing = al.itemSpacing;
    }
